...
author: "Andreas Jeppesen, Sophie Korch, Thea Pedersen og Lina Elkjær"
date: "03/09-2020"
...
Most variables should make sense, here are the less intuitive ones. 
...
#Excluding unnecessary collumns
demo_train <- select(demo_train , one_of("SUBJ" , "VISIT", "Diagnosis" , "Ethnicity" , "Gender" , "Age" , "ADOS" , "MullenRaw" , "ExpressiveLangRaw" , "Socialization" , "MOT_MLU" , "CHI_MLU" , "types_MOT" , "types_CHI" , "tokens_MOT" , "tokens_CHI"))
LU_train <- select(LU_train , one_of("SUBJ" , "VISIT", "Diagnosis" , "Ethnicity" , "Gender" , "Age" , "ADOS" , "MullenRaw" , "ExpressiveLangRaw" , "Socialization" , "MOT_MLU" , "CHI_MLU" , "types_MOT" , "types_CHI" , "tokens_MOT" , "tokens_CHI"))
token_train <- select(token_train , one_of("SUBJ" , "VISIT", "Diagnosis" , "Ethnicity" , "Gender" , "Age" , "ADOS" , "MullenRaw" , "ExpressiveLangRaw" , "Socialization" , "MOT_MLU" , "CHI_MLU" , "types_MOT" , "types_CHI" , "tokens_MOT" , "tokens_CHI"))


#Merging with more than 2 df's
df<- Reduce(function(...) merge(..., all=TRUE), list(demo_train, LU_train, token_train))


#Changing column names
names(df)[names(df) == 'ADOS'] <- 'SeverityOfSymptoms'
names(df)[names(df) == 'MullenRaw'] <- 'NonVerbalIQ'
names(df)[names(df) == 'ExpressiveLangRaw'] <- 'VerbalIQ'
names(df)[names(df) == 'Socialization'] <- 'SocialSkills'
```
...
#Creating subset including only visit 1
df1 <- subset(df, VISIT==1)


#Changing the names
names(df1)[names(df1) == 'SeverityOfSymptoms'] <- 'SeverityOfSymptoms1'
names(df1)[names(df1) == 'NonVerbalIQ'] <- 'NonVerbalIQ1'
names(df1)[names(df1) == 'VerbalIQ'] <- 'VerbalIQ1'
names(df1)[names(df1) == 'SocialSkills'] <- 'SocialSkills1'


#Selecting the columns we are gonna use
df1 <- select(df1,'SUBJ','SeverityOfSymptoms1','NonVerbalIQ1','VerbalIQ1','SocialSkills1')

#Merging
df <- merge(df1,df, by="SUBJ")
```
...
#Anonymizing participants by first changing to factor (thereby grouping the variables) then changing to numeric (every number now represents a participant)
df$SUBJ <- as.factor(df$SUBJ)
class(df$SUBJ)
df$SUBJ <- as.numeric(df$SUBJ)

#Changing the Gender values from 1 to M and from 2 to F 
df$Gender <- ifelse(df$Gender==1, "M", "F")

#Changing the diagnosis values from arbitrary values to meaningful values. ASD=autism spectrum disorder. TD= typically developing
df$Diagnosis <- ifelse(df$Diagnosis=="A", "ASD", "TD")

?write.csv

#Creating a csv with the dataframe
write.csv(df, file = "autismdata.csv")
